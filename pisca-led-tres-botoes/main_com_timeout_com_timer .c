...
// Variável para controlar o Alarme de Timeout (PASSO 1)7
volatile alarm_id_t alarme_timeout = -1;

// Função que roda quando dá 5 segundos de inatividade (PASSO 2)
...
void btn_callback(uint gpio, uint32_t events) {
    // Cancela o alarme antigo e cria um novo de 5000ms (PASSO 3)
    if (alarme_timeout != -1) {
      cancel_alarm(alarme_timeout);
    }
    alarme_timeout = add_alarm_in_ms(5000, timeout_callback, NULL, false);
...
int main() {
  ...
  // Inicia o primeiro alarme quando a placa liga
  alarme_timeout = add_alarm_in_ms(5000, timeout_callback, NULL, false);
